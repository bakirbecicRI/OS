...
//EXERCISE 5 helper print_pgdir implementation
static void
print_pgdir(pde_t *pgdir)
{
    ...
    for (uint32_t i = 0; i < NPDENTRIES; i++) {
        pde_t pde = pgdir[i];
        uint32_t va_start = i * PTSIZE;
        uint32_t va_end   = va_start + PTSIZE - 1;

        if (pde & PTE_P) {
            cprintf("PDE[%4d]  VA [0x%08x - 0x%08x]  PT 0x%08x |",
                    i, va_start, va_end, PTE_ADDR(pde));
            if (pde & PTE_W) cprintf(" W");
            if (pde & PTE_U) cprintf(" U");
            if (pde & PTE_P) cprintf(" P");
            cprintf("\n");
        } else {
            cprintf("PDE[%4d]  VA [0x%08x - 0x%08x] EMPTY\n",
                    i, va_start, va_end);
        }
    }
    cprintf("=======================================\n");
...
boot_alloc(uint32_t n)
{
  ...
  uint32_t used = (uint32_t)(nextfree-KERNBASE);
  uint32_t max = npages*PGSIZE;
...
page_init(void)
{
  ...
  //4. Extended memory: [EXTPHYSMEM, npages)
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  if ((*pte & perm) != perm || !(*pte & PTE_P) || !pte){
      if (addr < (uintptr_t) va){
        user_mem_check_addr = (uintptr_t) va;
        return -E_FAULT;
          }
